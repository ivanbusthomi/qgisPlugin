...
def pointAlongLine(interval, ppLine):
    # this function will create geometry point along line at the specified distance
    currentDistance = interval
    # return a geometry point along line
    iPoint = ppLine.interpolate(currentDistance)
    featPoint = QgsFeature()
    featPoint.setGeometry(iPoint)
    currentDistance = currentDistance + interval
    return featPoint                       # type feature

def distanceFromPoints(point1,point2):
    import math
    return math.sqrt(point1.sqrDist(point2))

def nextPointIteration(interval, ppLine, pointA, pointB):
    # this function will return the next start point candidate for perpendicularLine algorithm,
    # and return the previous 3-point combinations for circumcenter algorithm
    # derived from Qchainage by NathanW

    # container for feature that will be created
    feats = []
    ppPointGeom = pointAlongLine(interval,ppLine).geometry()
    ppPoint = ppPointGeom.asPoint()

    distance = distanceFromPoints(ppPoint,pointA)
    distanceB =distanceFromPoints(ppPoint,pointB)
    if distance != distanceB:
        raise ValueError
    buff = ppPointGeom.buffer(distance)
